...
- Harness adapters (ADR-0016): one MCP engine, four surfaces — `cursor`, `claude_code`, `codex`, `pi`. Layout table + detection live in `packages/pn-core-mcp/src/harness.ts` (mirrored at `pn-core://reference/harness-matrix.md`); MCP tools `harness_detect` / `harness_scaffold`; installer `scripts/install-to-project.mjs --harness <list|auto|all>` (imports `dist/harness.js`, so run `npm run build:mcp` first on a source checkout). Onboarding (`/pn-setup`, `/pn-new`, `project_kickoff`, `full_dev`) writes only the active harness's folders — never a default `.cursor/` on Claude Code, Codex, or Pi. Detection precedence: `PNCORE_HARNESS` > process env signals > workspace folders; Pi extension sets `PNCORE_HARNESS=pi`
...
- Response shortcodes: whole-token `scr` (compress last reply), `eli` (explain simply), `foc` (one signal), `ref` (rewrite as `F#`/`D#`/`O#`/`R#`/`Q#`/`A#`), `scp` (scope lock). Cold-session packet `project_context` includes compact `responseAliases`. When typed, also load `get_skill("pn-response-aliases")`. Rule `pn-communication-contract` stays `alwaysApply: false` (ADR-0013 / ADR-0017).
- Cursor plugin indexes every skill description (Codex catalog budget ~2% of context / 8000 characters for the whole list). MCP `list_skills` without filters is the Astra-compliant discovery path (category index). Emptying plugin `skills/` is deferred — ADR-0017.
