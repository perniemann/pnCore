...
- Consumer-project gating (ADR-0015): MCP gates `workflow_step` in chat only — it never owns the GitHub Merge button. `/pn-setup` / `/pn-new` in git repos write the trailer Cursor rule **and** portable `.githooks` (optional trailer-only Actions workflow, ask first). They do **not** copy this repo’s `pn-gates` / automerge / branch protection. Canonical: `pn-core://reference/consumer-gating.md`; installer `scripts/install-consumer-gating.mjs`
