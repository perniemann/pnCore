...
- Response shortcodes: whole-token `scr` (compress last reply), `eli` (explain simply), `foc` (one signal), `ref` (rewrite as `F#`/`D#`/`O#`/`R#`/`Q#`/`A#`), `scp` (scope lock). When typed, load `get_skill("pn-response-aliases")`. Rule `pn-communication-contract` stays `alwaysApply: false` (ADR-0013 / ADR-0017). Do not put the alias table on `project_context`.
