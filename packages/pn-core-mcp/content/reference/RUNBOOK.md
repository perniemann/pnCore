...
| Architecture / flowchart / sequence / loop / layers | `get_command("pn-diagram")` or `get_skill("pn-diagram-design")` | Mermaid-in-docs or editorial HTML/SVG. Floor: `pn-core://reference/diagram-baseline.md` (D-01–D-10 ship gate; skeptic + `pn-render-verify` on `/pn-diagram` visual track). No draw.io import in v1. |
