...
export function normalizeSessionModelSlug(sessionModel: string): string {
  return sessionModel
    .trim()
    .toLowerCase()
    .replace(/\s*\+\s*max\s*$/i, "")
...
function isDisplayNamePrefix(normSlug: string, normCandidate: string): boolean {
  if (!normCandidate.startsWith(`${normSlug}-`)) return false;
  const segments = slugSegments(normSlug);
  if (segments.length < 2) return false;
  if (/\d/.test(normSlug)) return false;
  return segments.every((s) => !VENDOR_PREFIX_DENYLIST.has(s));
}

function isPlausibleVendorPrefix(prefix: string): boolean {
  if (!prefix || prefix.includes("/")) return false;
  const segments = slugSegments(prefix);
  if (segments.length === 0) return false;
  if (!segments.every((s) => /^[a-z0-9]+$/.test(s))) return false;
  return segments.every((s) => !VENDOR_PREFIX_DENYLIST.has(s));
}

function hasVendorPrefixedSlug(normSlug: string, normCandidate: string): boolean {
  ...
  if (!normSlug.endsWith(`-${normCandidate}`)) return false;
 ...
 */
function slugMatchesTier(slug: string, candidate: string): boolean {
  const normSlug = normalizeSessionModelSlug(slug);
  const normCandidate = normalizeSessionModelSlug(candidate);
  if (normSlug === normCandidate) return true;
  if (isDisplayNamePrefix(normSlug, normCandidate)) return true;
  if (normSlug.startsWith(`${normCandidate}-`)) return true;
...
export function resolveSessionModelTier(sessionModel: string): ModelTier | null {
  ...
  for (const tier of Object.keys(TIER_META) as ModelTier[]) {
...
function resolveDeclaredLeadTier(state: LeadOrchestrationState): ModelTier | null {
  if (isModelTier(state.leadModelTier)) return state.leadModelTier;
...
function isPremiumTier(tier: ModelTier): boolean {
  return tier === "premium" || tier === "premium_thinking";
}

function buildSubagentTierHints(
  tierAliases?: Partial<Record<ModelTier, ModelTier>>
): Partial<Record<SubagentRole, SuggestedModelTier>> {
  const roles: SubagentRole[] = ["explorer", "builder", "checker"];
  const hints: Partial<Record<SubagentRole, SuggestedModelTier>> = {};
...
function formatSubagentHintLine(hints: Partial<Record<SubagentRole, SuggestedModelTier>>): string {
...
function buildLeadContract(
  mode: "lead" | "light_delegate",
  hints: Partial<Record<SubagentRole, SuggestedModelTier>>
): string {
  const subLine = formatSubagentHintLine(hints);
  if (mode === "lead") {
    return (
      "**Orchestrator lead mode (long_horizon):** Plan, coordinate, and verify only — do not bulk-edit files in the lead session. " +
  ...
  return (
    "**Light delegate mode:** Spawn Task subagents for each parallel task; lead verifies summaries before advancing. " +
 ...
 */
export function resolveLeadOrchestrationMode(
  state: LeadOrchestrationState,
  options: ResolveLeadOrchestrationOptions = {}
  ...
  let mode: OrchestrationMode = "implementer";
  ...
  const contractBlock =
    mode === "lead" || mode === "light_delegate" ? buildLeadContract(mode, hints) : "";
...
export function applyOrchestrationLead<
  T extends {
    instruction: string;
    parallel?: boolean;
    tasks?: Array<{ instruction: string; suggestedSubagentTier?: SuggestedModelTier }>;
  },
>(
  result: T,
  state: LeadOrchestrationState,
