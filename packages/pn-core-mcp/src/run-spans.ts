...
export function trailPaths(env: NodeJS.ProcessEnv = process.env): Record<TrailId, string> {
  return {
...
export type TrailRecord = Record<string, unknown> & { ts: string };
...
export function readTrail(
  ...
  runId: string,
  opts: { scanMax?: number } = {}
): { records: TrailRecord[]; path: string } | { error: string } {
  const safe = resolveSafePath(relPath);
  if ("error" in safe) return { error: safe.error };
  if (!existsSync(safe.resolved)) return { records: [], path: safe.resolved };
  ...
  for (const line of raw.split("\n")) {
    ...
    try {
      ...
      if (o.run_id !== runId && o.runId !== runId) continue;
...
type RunCursor = { stepIndex: number; lastTsMs: number };
const cursors = new Map<string, RunCursor>();

/** Test hook: forget every in-memory cursor. */
export function resetStepSpans(): void {
  cursors.clear();
}

/** Rebuild the cursor for `runId` from the run-log tail (after a server restart). */
export function recoverCursorFromLog(runId: string, logPath: string): RunCursor | null {
  const r = readTrail(logPath, runId);
  if ("error" in r || r.records.length === 0) return null;
  ...
  mine.forEach((e, i) => {
    ...
    if (Number.isFinite(ms) && ms > lastTsMs) lastTsMs = ms;
  });
 ...
 */
export function beginStepSpan(
  runId: string,
  opts: { nowMs?: number; logPath?: string } = {}
): StepSpanStart {
  const nowMs = opts.nowMs ?? Date.now();
  let cursor = cursors.get(runId);
  if (!cursor && opts.logPath) cursor = recoverCursorFromLog(runId, opts.logPath) ?? undefined;
...
export function currentStepIndex(runId: string | undefined): number | undefined {
...
export type TimelineLoad = { ts: string; tool: string; id: string };
...
export type RunTimeline = {
  ...
  slowest: { stepIndex: number; step: number; sinceLastStepMs: number } | null;
...
function stepIndexForTs(stepTsMs: number[], ts: string): number {
  const ms = tsMs(ts);
  if (!Number.isFinite(ms) || stepTsMs.length === 0) return stepTsMs.length - 1;
  let idx = -1;
  for (let i = 0; i < stepTsMs.length; i++) {
    if (stepTsMs[i] <= ms) idx = i;
    else break;
  }
  return idx < 0 ? 0 : idx;
}
...
export function buildRunTimeline(runId: string, trails: TimelineTrails): RunTimeline {
  const stepsSorted = [...trails.steps].sort((a, b) => a.ts.localeCompare(b.ts));
  const steps: TimelineStep[] = stepsSorted.map((e, i) => ({
  ...
  const slot = (rec: TrailRecord): TimelineStep | undefined => {
    if (steps.length === 0) return undefined;
    if (typeof rec.stepIndex === "number" && byIndex.has(rec.stepIndex)) {
      return steps[byIndex.get(rec.stepIndex) as number];
    }
  ...
  for (const l of trails.loads ?? []) {
    const s = slot(l);
    if (s) s.loads.push({ ts: l.ts, tool: String(l.tool ?? ""), id: String(l.id ?? "") });
  }
  ...
  for (const h of trails.handoff ?? []) {
    const s = slot(h);
    if (s && typeof h.summary === "string") s.handoff = h.summary.slice(0, 200);
  }
  ...
  let slowest: RunTimeline["slowest"] = null;
  ...
  const wallMs =
    first && last && Number.isFinite(tsMs(first)) && Number.isFinite(tsMs(last))
