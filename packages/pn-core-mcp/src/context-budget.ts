...
export function agentsMdCapBytes(env: NodeJS.ProcessEnv = process.env): number {
...
const KEEP_STATUSES = new Set<ArtifactReport["derived_status"]>([
  "drift",
  "missing",
  "in_progress",
  "unattested",
...
const PACK_HINT =
  "Packed to budget. For the omitted detail call project_context again with a larger max_tokens, or workflow_handoff_read / workflow_run_query for the trail.";

function measure(p: object): number {
...
function worthDropping(section: unknown): boolean {
...
export function packProjectContext(
...
): PackedProjectContext {
  const out: PackedProjectContext = {
    ...packet,
    budget: { maxTokens: null, estimatedTokens: 0, chars: 0, fits: true, steps: [] },
  };
  const finish = (): PackedProjectContext => {
    if (out.budget.steps.length === 0) delete out.budget.hint;
  ...
  if (maxTokens == null || !Number.isFinite(maxTokens) || maxTokens <= 0) return finish();
  ...
  const begin = (action: string, placeholder: string): { done: (detail: string) => void } => {
    const step: PackStep = { action, detail: placeholder, savedChars: 0 };
  ...
  if (out.trail && out.trail.length > 3) {
    ...
    while (out.trail.length > 3 && over()) out.trail = out.trail.slice(1);
  ...
  if (out.artifacts?.some((a) => !KEEP_STATUSES.has(a.derived_status) && a.notes.length > 0)) {
    ...
    out.artifacts = out.artifacts.map((a) => {
      if (KEEP_STATUSES.has(a.derived_status) || a.notes.length === 0) return a;
      cleared++;
      return { ...a, notes: [] };
    });
  ...
  if (out.artifacts?.some((a) => !KEEP_STATUSES.has(a.derived_status))) {
    const step = begin(
      ...
      "dropped 00 settled artifacts (kept drift/missing/in_progress/unattested)"
    );
    const omitted: Record<string, number> = {};
    out.artifacts = out.artifacts.filter((a) => {
      if (KEEP_STATUSES.has(a.derived_status)) return true;
  ...
  if (out.artifacts && out.artifacts.length > 0) {
    ...
    const step = begin(
      "artifacts.drop_all",
      `dropped remaining ${n} artifacts; drift ids stay under drift[]`
    );
    const omitted: Record<string, number> = { ...(out.artifacts_omitted ?? {}) };
    for (const a of out.artifacts) omitted[a.derived_status] = (omitted[a.derived_status] ?? 0) + 1;
    out.artifacts = [];
