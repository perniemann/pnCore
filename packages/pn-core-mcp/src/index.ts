 ...
 */
function getContentMaxChars(): number {
...
const server = new McpServer(
  { name: "pn-core-mcp", version: _mcpVersion },
  { capabilities: { prompts: {}, resources: {} } }
);

// Local typed wrapper around server.tool().
//
// Why this exists: @modelcontextprotocol/sdk@1.27 declares its `tool` callback
// type as `Args extends ZodRawShapeCompat ? (args: ShapeOutput<Args>, ...) ...`
// where ShapeOutput maps over every shape key with a v3-OR-v4 union inference
// (see node_modules/@modelcontextprotocol/sdk/dist/esm/server/zod-compat.d.ts).
// Combined with zod 3.25+'s dual v3/v4 type tree, inferring this for every
// inline schema in this file blows the TS instantiation budget and triggers
// TS2589 (and OOM in tsc). Routing every call through `regTool` collapses
// inference to a single, pure-v3 `z.infer` per key per call site.
type RawShape = Record<string, z.ZodTypeAny>;
type ShapeArgs<S extends RawShape> = { [K in keyof S]: z.infer<S[K]> };
...
function regTool<S extends RawShape>(
  name: string,
  description: string,
  schema: S,
  annotations: ToolAnnotations,
  handler: (args: ShapeArgs<S>) => unknown | Promise<unknown>
): void {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (server.tool as any)(name, description, schema, annotations, handler);
}

// Same TS2589 escape hatch for server.prompt (PromptArgsRawShape == ZodRawShapeCompat).
function regPrompt<S extends RawShape>(
  name: string,
  description: string,
  schema: S,
  handler: (args: ShapeArgs<S>) => unknown | Promise<unknown>
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (server.prompt as any)(name, description, schema, handler);
}
 ...
 */
const ERROR_CODE_TO_JSON_RPC: Record<keyof typeof ErrorCode, number> = {
  NOT_FOUND: -32004,
  INVALID_STATE: -32600,
...
const requiredHumanGateWorkflows = parseRequiredApprovalWorkflows(
  process.env.PNCORE_REQUIRE_APPROVAL_FOR_WORKFLOWS
);
const defaultHumanGateTicketsPath =
...
function resolveSafePath(filePath: string): { resolved: string } | { error: string } {
...
function errJson(code: keyof typeof ErrorCode, message: string, extra?: Record<string, unknown>) {
 ...
 */
function mcpError(code: keyof typeof ErrorCode, message: string, extra?: Record<string, unknown>) {
...
function promptMessages(content: string, context?: string) {
...
regTool(
  "suggest_model_tier",
  "Return the suggested LLM model tier for a workflow step. Omit `step` to get the full per-step table for the workflow. Tiers: fast | standard | premium | premium_thinking. Suggestions reflect the cognitive demand of each step and incorporate any modelTierOverrides / tierAliases configured in features.json or PNCORE_FEATURES.",
  {
...
regTool(
  ...
  "List pnCore skills. Without filters returns a category index with counts; use category or filter to drill in.",
  {
    category: z
      ...
      .optional()
      .describe(
        "Filter by category folder (e.g. frontend, backend, gamedev, media, orchestration, ci, discipline, integrations, learning, marketing, pm, review, support, plugin)"
      ),
    filter: z
      .string()
      .optional()
...
regTool(
  ...
  "Return full markdown content of a skill by id (e.g. pn-discovery-questionnaire, pn-svg-creator).",
  {
    id: z
      .string()
      .describe(
        "Skill id (e.g. pn-discovery-questionnaire, pn-writing-plans, pn-svg-creator, pn-frontend-design-philosophy)"
      ),
    run_id: runIdOptArg,
  },
  { readOnlyHint: true, destructiveHint: false, idempotentHint: true },
  async ({ id, run_id }) => {
    ...
    if (shouldLog) {
      try {
        ...
        appendFileSync(
          logPath,
          JSON.stringify({ ts: new Date().toISOString(), id, ...(run_id ? { run_id } : {}) }) +
            "\n",
          "utf-8"
        );
      } catch (err) {
...
const listGetTools = [
  ...
  },
  {
    ...
    idDesc:
      "Rule id (e.g. pn-react, pn-nextjs, pn-build-gate, pn-mcp-proactive, pn-no-cursor-commit-trailers)",
    notFoundLabel: "Rule",
    list: listRules,
    get: getRule,
  },
...
regTool(
  ...
  "List pnCore agent ids and descriptions. Use include='all' to surface internal orchestration agents.",
  {
    include: z
      ...
      .optional()
      .describe(
        "Which agents to include: 'external' (default, public-facing), 'internal' (orchestration-only), or 'all'"
      ),
  },
  { readOnlyHint: true, destructiveHint: false, idempotentHint: true },
  async ({ include }) => {
...
regTool(
  ...
  "Return full markdown content of an agent by id (e.g. pn-project-builder, pn-skeptic).",
  {
    id: z
      .string()
      .describe(
        "Agent id (e.g. pn-project-builder, pn-reviewer, pn-skeptic, pn-frontend-developer)"
      ),
    run_id: runIdOptArg,
  },
  { readOnlyHint: true, destructiveHint: false, idempotentHint: true },
...
for (const {
  ...
  regTool(
    ...
    {},
    { readOnlyHint: true, destructiveHint: false, idempotentHint: true },
    async () => textContent(JSON.stringify(list()))
  );
  regTool(
    getName,
    getDesc,
    { id: z.string().describe(idDesc), run_id: runIdOptArg },
    { readOnlyHint: true, destructiveHint: false, idempotentHint: true },
    async ({ id, run_id }) => {
      let content = get(id);
      if (content == null)
...
regTool(
  ...
  "Report token and cost usage for a workflow step. Call after workflow_step when the client has usage data. Optional path: append one JSON line to the file (e.g. .pncore/usage.jsonl in workspace). If path omitted, returns summary only.",
  {
    ...
    path: z
      ...
      .optional()
      .describe(
        "Optional file path to append to (e.g. .pncore/usage.jsonl); must be within process cwd"
      ),
  },
  { readOnlyHint: false, destructiveHint: false, idempotentHint: false },
  async ({
...
regTool(
  ...
  { readOnlyHint: false, destructiveHint: false, idempotentHint: false },
  async ({ run_id, step, summary, path: filePath }) => {
...
regTool(
  ...
  { readOnlyHint: true, destructiveHint: false, idempotentHint: true },
  async ({ run_id, path: filePath, max_lines }) => {
    const rel = filePath ?? defaultHandoffPath;
    const safe = resolveSafePath(rel);
    if ("error" in safe) return mcpError("PATH_TRAVERSAL", safe.error, { path: rel });
    const resolved = safe.resolved;
...
regTool(
  ...
  "Present a structured confirmation gate to the user. Use when ask_question is unavailable (MCP-only), or after AskQuestion in Cursor to record gate_id in .pncore/gate-log.jsonl. Returns formatted prompt text, options, and gate_id. You MUST output the prompt to the user, list the options, and STOP. Do not proceed until the user replies. For skeptic gates: set gate_type=skeptic, verdict, and non-empty context when verdict=revise.",
  {
    question: z
      .string()
      .describe(
        "The confirmation question to present (e.g. 'Skeptic pass: apply revisions and proceed?')"
      ),
    options: z
      .array(z.string())
      .min(1)
      .max(10)
      .describe(
        "Option labels (e.g. ['proceed', 'revise_plan', 'add_correction']). Avoid yes/no-only when real choices exist."
      ),
  ...
  async ({ question, options, context, gate_type, verdict, must_fix_summary, path: filePath }) => {
    const gateInput = {
      question,
      options,
      context,
...
regTool(
  ...
  "Hard approval gate: returns success only if approval_token equals PNCORE_APPROVAL_TOKEN on the MCP server process. Configure the env var in MCP server config (not in chat). Optional workflow_type + workflow_step: when both are set and the token matches, appends a one-time ticket and returns pncoreHumanGateTicket for the next workflow_step human gate (used with PNCORE_REQUIRE_APPROVAL_FOR_WORKFLOWS). If the env var is unset, returns an error (fail closed).",
  {
    ...
    workflow_type: workflowTypeEnum
      .optional()
      .describe(
        "Optional. When set with workflow_step, issues a human-gate ticket for workflow_step after token validates."
      ),
    workflow_step: z
      .number()
      .int()
  ...
  { readOnlyHint: false, destructiveHint: false, idempotentHint: false },
  async ({ approval_token, action_label, workflow_type, workflow_step, run_id }) => {
...
regTool(
  ...
  { readOnlyHint: false, destructiveHint: false, idempotentHint: false },
  async ({ gate_type, workflowType, step, outcome, action_label, run_id, path: filePath }) => {
...
regTool(
  ...
  "Save workflow state to a file so it can be restored later (resume after disconnect). Path defaults to .pncore/workflow-state.json relative to process cwd.",
  {
    ...
    path: z
      ...
      .optional()
      .describe(
        "Optional file path; default .pncore/workflow-state.json; must be within process cwd"
      ),
  },
  { readOnlyHint: false, destructiveHint: false, idempotentHint: true },
  async ({ state, path: filePath }) => {
...
for (const { id, name, description } of listAgents()) {
  regPrompt(id, description || `Run the ${name} agent`, contextArgSchema, ({ context }) => {
    ...
    if (content == null) throwMcpError("NOT_FOUND", `Agent not found: ${id}`, { id });
    return promptMessages(content, context);
  });
}

for (const { id, name, description } of listCommands()) {
  if (agentIds.has(id)) continue; // skip — already registered as agent prompt
  regPrompt(id, description || `Run the ${name} command`, contextArgSchema, ({ context }) => {
    const content = getCommand(id);
    if (content == null) throwMcpError("NOT_FOUND", `Command not found: ${id}`, { id });
    return promptMessages(content, context);
  });
}
