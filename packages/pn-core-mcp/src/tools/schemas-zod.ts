...
export const harnessDetectSchema = {
  harness: harnessIdEnum
    .optional()
    .describe(
      "Return the layout for one harness only. Omit to detect the active harness(es) and return all layouts."
    ),
} as const;
...
export const harnessScaffoldSchema = {
  harnesses: z
    ...
    .optional()
    .describe(
      "Target harnesses. Default: detected via PNCORE_HARNESS, process env, then workspace folders. Error when nothing is detected and this is omitted."
    ),
  project: z
    .object({
      name: z.string().min(1),
  ...
  include: z
    ...
    .optional()
    .describe(
      "Artifacts to generate. Default: project_context, project_skill, no_trailers_rule. mcp_config adds the pn-core server entry (Cursor .cursor/mcp.json, Claude .mcp.json, Codex .codex/config.toml, Pi .pi/settings.json packages)."
    ),
  dryRun: z
    .boolean()
    .optional()
...
export const listSkillsSchema = {
  category: z
    ...
    .optional()
    .describe(
      "Filter by category folder (e.g. frontend, backend, gamedev, media, orchestration, ci, discipline, integrations, learning, marketing, pm, review, support, plugin, fsi)"
    ),
  filter: z
    .string()
    .optional()
...
export const listCommandsSchema = {} as const;
...
export const listRulesSchema = {} as const;
...
export const workflowRunQuerySchema = {
  ...
  kinds: z
    ...
    .optional()
    .describe(
      "Event kinds to include (default: verify + acceptance). step = workflow_step spans from workflow-runs.jsonl (stepIndex, sinceLastStepMs, engineMs, routing); load = get_* loads from skill-load-log.jsonl; usage / handoff / gate = the matching .pncore trails."
    ),
  ...
  timeline: z
    ...
    .optional()
    .describe(
      "Also return a per-step timeline joining every trail by run_id: loads, usage, handoff, gates, verify per step; totals; slowest step by agent wall time; wallMs; accepted."
    ),
} as const;
