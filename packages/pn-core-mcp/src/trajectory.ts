...
import { getWorkflowStep, workflowSteps, type GateType, type WorkflowType } from "./workflows.js";
...
export function runLogStateEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
 ...
 */
export function snapshotStateForLog(
  state: Record<string, unknown>,
  opts: { maxString?: number; maxDepth?: number } = {}
  ...
  const walk = (v: unknown, depth: number, key?: string): unknown => {
    if (key && REDACTED_KEYS.has(key) && v != null) return "[redacted]";
    if (typeof v === "string") {
  ...
  return walk(state, 0) as Record<string, unknown>;
}

export function parseRunLog(text: string): { entries: RunLogEntry[]; skipped: number } {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    if (!t) continue;
    try {
      const o = JSON.parse(t) as Partial<RunLogEntry>;
      if (
        typeof o.workflowType === "string" &&
        ...
        entries.push({
          ...
          workflowType: o.workflowType as WorkflowType,
          step: o.step,
          nextStep: o.nextStep,
          gate: o.gate,
          done: o.done === true,
...
export function groupRunLogByRun(entries: RunLogEntry[]): Map<string, RunLogEntry[]> {
  const indexed = new Map<string, Array<[number, RunLogEntry]>>();
  entries.forEach((e, i) => {
...
export type TrajectoryExpectError = { error: string };
...
export function isErrorExpect(
  e: TrajectoryExpect | TrajectoryExpectError
): e is TrajectoryExpectError {
  return "error" in e;
}
 ...
 */
export function trajectoryFromRunLog(
  entries: RunLogEntry[],
  opts: { name: string; runId?: string }
): Trajectory {
  if (entries.length === 0) throw new Error("trajectoryFromRunLog: no entries for this run");
  const types = new Set(entries.map((e) => e.workflowType));
  if (types.size !== 1) {
    throw new Error(
      `trajectoryFromRunLog: entries span several workflow types (${[...types].join(", ")}); a trajectory covers one workflow`
    );
  }
  const missing = entries.filter((e) => !e.state);
  if (missing.length > 0) {
    throw new Error(
      `trajectoryFromRunLog: ${missing.length} of ${entries.length} entries have no state snapshot. Record with ${RUN_LOG_STATE_ENV}=1 in the MCP server env.`
    );
  }
  return {
    name: opts.name,
    ...
    steps: entries.map((e) => {
      const expect: TrajectoryExpect = { nextStep: e.nextStep, gate: e.gate, done: e.done };
      if (e.workflowPhase) expect.workflowPhase = e.workflowPhase;
      if (e.parallel) expect.parallel = true;
      if (e.taskIds) expect.taskIds = e.taskIds;
      return { step: e.step, state: e.state as Record<string, unknown>, expect };
    }),
  };
}
...
function sortedIds(ids: string[] | undefined): string[] | undefined {
...
export function replayTrajectory(t: Trajectory): ReplayResult {
  const mismatches: TrajectoryMismatch[] = [];
  t.steps.forEach((s, index) => {
    ...
    const push = (field: string, expected: unknown, actual: unknown) =>
      mismatches.push({ index, step: s.step, field, expected, actual });

    if (isErrorExpect(s.expect)) {
      if (!("error" in r)) push("error", s.expect.error, "(no error; step succeeded)");
      else if (!r.error.includes(s.expect.error)) push("error", s.expect.error, r.error);
      return;
    }
    if ("error" in r) {
    ...
    const e = s.expect;
    if (r.nextStep !== e.nextStep) push("nextStep", e.nextStep, r.nextStep);
    if (r.gate !== e.gate) push("gate", e.gate, r.gate);
    if ((r.done ?? false) !== e.done) push("done", e.done, r.done ?? false);
    ...
    if (e.taskIds) {
      ...
      const expected = sortedIds(e.taskIds) as string[];
      if (JSON.stringify(actual) !== JSON.stringify(expected)) push("taskIds", expected, actual);
    }
    for (const sub of e.instructionContains ?? []) {
      if (!r.instruction.includes(sub)) push("instructionContains", sub, "(absent)");
    }
  });
  return {
    name: t.name,
 ...
 */
export function checkTrajectoryContinuity(t: Trajectory): string[] {
  const problems: string[] = [];
  const steps = workflowSteps[t.workflowType];
  if (!steps) return [`unknown workflowType ${t.workflowType}`];
  if (t.steps.length === 0) problems.push("trajectory has no steps");
  t.steps.forEach((s, i) => {
    if (s.step < 0 || s.step >= steps.length) {
      problems.push(
        `#${i}: step ${s.step} out of range for ${t.workflowType} (0..${steps.length - 1})`
      );
    }
    if (i === 0) return;
    ...
    if (!allowed.includes(s.step)) {
      problems.push(
        `#${i}: step ${s.step} does not continue from #${i - 1} (step ${prev.step} → expected ${allowed.join(" or ")})`
      );
    }
...
export function parseTrajectory(raw: unknown, sourceLabel = "trajectory"): Trajectory {
  ...
  o.steps.forEach((s, i) => {
    ...
    const ex = st.expect as Record<string, unknown> | undefined;
    ...
    if (
      typeof ex.nextStep !== "number" ||
      (ex.gate !== "human" && ex.gate !== "model") ||
      typeof ex.done !== "boolean"
    ) {
      throw new Error(
        `${sourceLabel}: steps[${i}].expect needs nextStep:number, gate:human|model, done:boolean`
      );
    }
  });
