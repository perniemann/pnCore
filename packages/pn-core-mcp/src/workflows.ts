...
const imageCreateSteps: StepDef[] = [
  ...
  {
    instruction:
      "REQUIRED: Run get_skill('pn-render-verify') on outputPath, then get_skill('pn-skeptic-challenge') post-build on the image. Output skeptic verdict. After user confirms, call workflow_step(step=4) with state: { skepticOutputPassed: <gate record from workflow_confirm>, skepticOutputVerdict }." +
      GATE_STATE_FROM_CONFIRM,
    gate: "human",
    nextStep: 4,
  ...
  {
    instruction:
      "Output summary: what was created, path, format, skeptic verdict. Image_create complete." +
      paperclipWorkflowHint() +
      " Do not call workflow_step again.",
    gate: "model",
    nextStep: 4,
...
export const INTERNAL_ENGINE_WORKFLOW_TYPES = ["unreal_feature", "godot_feature"] as const;
...
export const PUBLIC_WORKFLOW_TYPES = (Object.keys(workflowSteps) as WorkflowType[]).filter(
  (t) => !(INTERNAL_ENGINE_WORKFLOW_TYPES as readonly string[]).includes(t)
);
