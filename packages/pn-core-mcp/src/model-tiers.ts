...
export const TIER_META: Record<ModelTier, TierMeta> = {
  ...
  long_horizon: {
    ...
    description:
      "Long-horizon tier: multi-hour loop orchestration, sustained planning, escalation after cheap-tier verify failures (Anthropic Fable 5.1). Opus remains the daily-driver premium.",
  },
};
...
export function resolveRoleTier(
  ...
  const roleRationale: Record<SubagentRole, string> = {
    ...
    orchestrator:
      "Long-horizon loop lead: sustained orchestration, escalation queue, multi-hour scheduled runs (Fable 5.1).",
  };
...
export const FRONTIER_ROUTER_SENTENCE =
  "Load matching skills as routers; do not preload unrelated recipes; run local disposable tests without asking; stop only for irreversible or option-lock gates.";

const FRONTIER_HINT_TIERS: ReadonlySet<ModelTier> = new Set([
  "premium",
  "premium_thinking",
  "long_horizon",
]);
