...
export function assertSafeArgv(argv: string[], opts?: { freeform?: boolean }): void {
...
export function resolveSandboxBackend(): SandboxBackend {
  ...
  if (forced === "unavailable") return "unavailable";
  if (forced === "test") {
    return inTest ? "test" : "unavailable";
  }
  if (forced === "restricted") return "restricted";
  if (forced === "bwrap" || forced === "bubblewrap") {
    return commandExists("bwrap") ? "bubblewrap" : "unavailable";
  }
  if (!forced && commandExists("bwrap")) return "bubblewrap";
  if (!forced && inTest) return "test";
  return "unavailable";
}
...
export function tailOutput(buf: string): string {
  if (buf.length <= OUTPUT_CAP) return buf;
  return buf.slice(buf.length - OUTPUT_CAP);
}

export function buildBwrapArgv(cwd: string, argv: string[]): string[] {
...
export function spawnVerify(opts: {
  ...
  if (backend === "unavailable") {
    return Promise.reject(
      new VerifyPolicyError(
        "DISPOSE_UNAVAILABLE",
        "Dispose-verify jail is unavailable. Set PNCORE_VERIFY_SANDBOX=restricted to opt into no-shell spawn without an OS jail, or install bwrap."
      )
    );
  }
...
export function sandboxLabel(backend: SandboxBackend): {
  backend: "bubblewrap" | "docker" | "seatbelt" | "unavailable";
  jailed: boolean;
} {
  if (backend === "bubblewrap") return { backend: "bubblewrap", jailed: true };
  if (backend === "unavailable") return { backend: "unavailable", jailed: false };
