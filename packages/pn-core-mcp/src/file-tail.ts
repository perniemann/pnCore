 ...
 */
export function readFileTail(resolvedPath: string, maxBytes: number): string {
  const st = statSync(resolvedPath);
  if (st.size <= maxBytes) return readFileSync(resolvedPath, "utf-8");
...
export function tailScanBytesFromEnv(
