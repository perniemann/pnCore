...
let _cfg: DebugConfig | null = null;
...
export function __resetDebugConfigForTests(): void {
  _cfg = null;
}
 ...
 */
export function debug(channel: string, msg: string, extra?: Record<string, unknown>): void {
  const cfg = getConfig();
  if (!cfg.enabled) return;
  if (!cfg.all && !cfg.channels!.has(channel)) return;
