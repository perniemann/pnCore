...
const humanGateScanMaxBytes = tailScanBytesFromEnv(process.env.PNCORE_HUMAN_GATE_SCAN_BYTES, 768 * 1024);
const TICKET_TTL_MS = 24 * 60 * 60 * 1000;
const knownWorkflowTypes = new Set(Object.keys(workflowSteps));
...
export function parseRequiredApprovalWorkflows(raw) {
    ...
    for (const part of raw.split(",")) {
        ...
        if (!knownWorkflowTypes.has(w)) {
            debug("tickets", `PNCORE_REQUIRE_APPROVAL_FOR_WORKFLOWS: unknown workflow type "${w}" — ignored. Known types: ${[...knownWorkflowTypes].join(", ")}`);
            continue;
        }
        out.add(w);
    }
...
export function workflowRequiresHumanGateApproval(required, workflowType) {
...
export function validateAndConsumeHumanGateTicket(filePath, workflowType, step, ticket, runIdFromState) {
    if (ticket === undefined || ticket === "") {
        return {
            ok: false,
            code: "APPROVAL_REQUIRED",
            reason: "This workflow step is a human gate with mandatory approval. Call approval_checkpoint with the same approval_token, action_label, workflow_type, and workflow_step, then pass the returned pncoreHumanGateTicket in state on workflow_step.",
        };
    }
    ...
    for (const L of lines) {
        if (L.type === "consumed")
            consumed.add(L.ticket);
    }
    if (consumed.has(ticket)) {
        return {
            ok: false,
            code: "INVALID_STATE",
            reason: "Human gate ticket was already consumed. Call approval_checkpoint again for this step.",
        };
    }
    const now = Date.now();
    let found = null;
    for (const L of lines) {
        ...
        if (L.ticket !== ticket)
            continue;
        if (L.workflowType !== workflowType || L.step !== step)
            continue;
    ...
    if (!found) {
        return {
            ok: false,
            code: "APPROVAL_REQUIRED",
            reason: "No valid human gate ticket for this workflowType and step. Call approval_checkpoint with matching workflow_type and workflow_step (and correct approval_token).",
        };
    }
    const consumedLine = {
        v: 1,
