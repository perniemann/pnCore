...
export const projectContextSchema = {
    ...
    max_tokens: z
        ...
        .max(200_000)
        .optional()
        .describe("Token budget (chars ÷ 4) for the packet. When set, sections are shrunk/dropped in a fixed order (trail oldest-first, settled artifacts' notes, settled artifacts, pointers, trail, artifacts, drift notes) until it fits; `budget.steps` lists what was cut. Drift, next_incomplete, resume, counts are never dropped."),
};
...
export const listSkillsSchema = {
    category: z
        .string()
        .optional()
        .describe("Filter by category folder (e.g. frontend, backend, gamedev, media, orchestration, ci, discipline, integrations, learning, marketing, pm, review, support, plugin, fsi)"),
    filter: z
        .string()
        .optional()
