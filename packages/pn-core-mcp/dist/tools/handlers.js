...
export async function handleHarnessDetect(args) {
    ...
    return textContent(JSON.stringify({
        ...detection,
        layouts: layoutTable(),
        hint: detection.detected.length === 0
...
export async function handleHarnessScaffold(args) {
    const detection = detectHarness({ cwd: safeBase });
    const harnesses = args.harnesses && args.harnesses.length > 0 ? args.harnesses : detection.detected;
    if (harnesses.length === 0) {
        return mcpError("INVALID_STATE", "No harness selected and none detected. Pass harnesses: [cursor | claude_code | codex | pi].", { detection });
    }
    ...
    try {
        ...
        if (msg.includes("escapes workspace"))
            return mcpError("PATH_TRAVERSAL", msg, {});
...
export async function handleWorkflowRunQuery(args) {
    ...
    const trailRecords = {};
    const readTrailOrFail = (trail) => {
        if (trailRecords[trail])
            return trailRecords[trail];
    ...
    let merged = result.events.filter((e) => kinds.includes(e.kind));
    let timeline;
    try {
        for (const k of kinds) {
            if (k === "verify" || k === "acceptance")
                continue;
            ...
            for (const rec of readTrailOrFail(trail)) {
                ...
                const { state: _state, runId: _runId, ...rest } = rec;
                void _state;
                void _runId;
        ...
        if (args.timeline) {
            ...
            timeline = buildRunTimeline(runId, {
                steps: entries,
                loads: readTrailOrFail("loads"),
                usage: readTrailOrFail("usage"),
                handoff: readTrailOrFail("handoff"),
                gate: readTrailOrFail("gate"),
