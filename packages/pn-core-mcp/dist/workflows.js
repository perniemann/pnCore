...
const designSteps = [
    ...
    {
        instruction: "REQUIRED: Run get_skill('pn-skeptic-challenge') in post-build mode. Review output against plan and philosophy. Output skeptic verdict. After user confirms, call workflow_step(step=5) with state: { skepticOutputPassed: true, skepticOutputVerdict }.",
        gate: "human",
        nextStep: 5,
...
const promptOptimizeSteps = [
    ...
    {
        instruction: "From promptSpec, produce draft optimized prompt (4-Block layout per pn-prompt-optimize). Output: draft, notes, usage tips. After user confirms or provides feedback, call workflow_step(step=2) with state: { draft, notes, usage, reviewComplete: true, userFeedback? }.",
        gate: "human",
        nextStep: 2,
...
const gameFeatureSteps = [
    ...
    {
        instruction: "Create implementation plan (game loop fit, state changes, integration points). Run get_skill('pn-skeptic-challenge') on the plan. Output both. After user confirms, call workflow_step(step=2) with state: { plan, skepticPassed: true }.",
        gate: "human",
        nextStep: 2,
    ...
    {
        instruction: "REQUIRED: Run get_skill('pn-skeptic-challenge') in post-build mode on the implemented game mechanic. Review output against plan and game logic requirements. Output skeptic verdict. After user confirms, call workflow_step(step=4) with state: { skepticOutputPassed: true, skepticOutputVerdict }.",
        gate: "human",
        nextStep: 4,
...
const svgCreateSteps = [
    ...
    {
        instruction: "REQUIRED: Run get_skill('pn-skeptic-challenge') post-build on the SVG. Output skeptic verdict. After user confirms, call workflow_step(step=4) with state: { skepticOutputPassed: true, skepticOutputVerdict }.",
        gate: "human",
        nextStep: 4,
