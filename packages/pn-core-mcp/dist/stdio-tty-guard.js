 ...
 */
export const TTY_USAGE = "pn-core is a stdio MCP server (no interactive prompt). " +
...
export function isInteractiveStdin(stdin = process.stdin, env = process.env) {
    if (env.PNCORE_MCP_ALLOW_TTY === "1")
        return false;
...
export function exitIfInteractiveStdin(stdin = process.stdin, stderr = process.stderr, env = process.env, exitFn = defaultExit) {
    if (!isInteractiveStdin(stdin, env))
        return false;
