...
MCP server for [pnCore](https://github.com/perniemann/pnCore) **0.17.0**: same skills, agents, commands, and rules as the Cursor plugin, plus **`workflow_step`** and related tools. Use from any MCP client to run orchestration, discovery, skeptic, audits, assets, and other pnCore workflows without installing the plugin.
...
| **suggest_model_tier** | read | Suggested LLM model tier for a workflow step (`fast` / `standard` / `premium` / `premium_thinking`). Omit `step` to get the full per-step table. Resolves `PNCORE_FEATURES.modelTierOverrides` and `tierAliases`. |
