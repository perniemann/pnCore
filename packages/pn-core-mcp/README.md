...
MCP server for [pnCore](https://github.com/perniemann/pnCore) **0.19.3**: same skills, agents, commands, and rules as the Cursor plugin, plus **`workflow_step`** and related tools. Use from any MCP client to run orchestration, discovery, skeptic, audits, assets, and other pnCore workflows without installing the plugin.
...
| **project_context** | read | Cold-session packet from `docs/refs/context-index.json` + `.pncore` trails (`mode` operator / agent). Every packet carries `budget` (chars, tokens ≈ chars ÷ 4). **`max_tokens`** packs it deterministically — trail oldest-first, settled artifacts' notes, settled artifacts, pointers, trail, artifacts, drift notes — and lists the cuts in `budget.steps` (ADR-0019). Drift, `next_incomplete`, `resume`, `counts` are never dropped. |
