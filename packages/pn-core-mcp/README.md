...
MCP server for [pnCore](https://github.com/perniemann/pnCore) **0.19.5**: same skills, agents, commands, and rules as the Cursor plugin, plus **`workflow_step`** and related tools. Use from any MCP client to run orchestration, discovery, skeptic, audits, assets, and other pnCore workflows without installing the plugin.
...
| **project_context** | read | Cold-session packet from `docs/refs/context-index.json` + `.pncore` trails (`mode` operator / agent). Every packet carries `budget` (chars, tokens ≈ chars ÷ 4). **`max_tokens`** packs it deterministically — trail oldest-first, settled artifacts' notes, settled artifacts, pointers, trail, artifacts, drift notes — and lists the cuts in `budget.steps` (ADR-0021). Drift, `next_incomplete`, `resume`, `counts` are never dropped. |
...
| **workflow_run_query** | read | Query one **`run_id`** across the `.pncore` trails: verify/acceptance (default), `kinds` `step` (spans: `stepIndex`, `sinceLastStepMs`, `engineMs`, routing), `load`, `usage`, `handoff`, `gate`; `timeline: true` joins them per step with totals and the slowest step (ADR-0020). |
...
### Context budget (ADR-0021)

Measurement is `chars ÷ 4` everywhere (`scripts/measure-tokens.mjs`, dashboard, `project_context.budget`). Packing exists in the two places with a hard limit: `project_context` with `max_tokens` (fixed drop order, cuts listed in `budget.steps`, `fits` reported honestly when the core alone is over) and the Codex `AGENTS.md` chain, where `plugin-install --harness codex --inline-rules` inlines always-on rules in priority order (`ALWAYS_ON_RULE_PRIORITY` in `src/harness.ts`) and leaves the lowest-priority ones out as `get_rule` pointers so the whole file stays under the cap; content outside the pnCore block is never touched, and a file that is over the cap on its own gets a warning.
