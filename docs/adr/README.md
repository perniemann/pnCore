...
- [0020-step-spans.md](./0020-step-spans.md) — step spans on the run log (`stepIndex`, `sinceLastStepMs`, `engineMs`), loads tagged per step, `workflow_run_query` joins the `.pncore` trails into a timeline; local JSONL, no OTel backend
- [0021-context-budget.md](./0021-context-budget.md) — narrow context packing on the existing chars ÷ 4 measurement: `project_context` `max_tokens` with a fixed drop order, and the Codex `AGENTS.md` 32 KiB cap honoured by priority-ordered rule inlining; no general assembler
