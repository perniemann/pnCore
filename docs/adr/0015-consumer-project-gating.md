...
- **Negative:** Hooks are still local-clone config (`core.hooksPath`). CI is opt-in. A human with write access can still merge a red PR. Mitigation: the reference states that GitHub branch protection is the adopter’s repo policy, not an MCP feature.
