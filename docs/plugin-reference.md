...
- **pn-setup** — Configure pnCore for an existing project. Choose: (1) Everything, (2) Project integration only (codebase analysis, project-context.mdc, project skill, file-glob rules), (3) Design context only (`.pncore-design.md`; optional house philosophy, primary reference URL, diagram tokens, CLAUDE.md aesthetics block), (4) Stack context only (.pncore-stack.md). Section A git projects also get the trailer Cursor rule plus portable `.githooks` (`core.hooksPath` set only when unset; compose into an existing hook manager). Optional trailer-only Actions workflow — ask first. Template: `.cursor/docs/templates/pncore-design.example.md` after install. Gating split: `pn-core://reference/consumer-gating.md`.
