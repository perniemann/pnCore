...
- **pn-assets-manager** — SVG, raster images, logos, placeholders; routes to workflow_step("svg_create")/inline SVG flow or workflow_step("image_create")/inline image flow, or placeholder URLs.
...
- **pn-assets** — Single entry for any visual asset. Routes to SVG flow (`workflow_step("svg_create")` or inline) or image flow (`workflow_step("image_create")` or inline), or placeholder URLs. Content discussed before generation.
