...
If AGENTS.md says local tests use disposable fixtures and have no production access, the agent may run them without asking. Irreversible work, option-lock, and phase-complete still use `pn-build-gate` / `approval_checkpoint`. Premium+ `renderTierHint` appends one router/local-tests sentence; do not duplicate that in always-on rules.
