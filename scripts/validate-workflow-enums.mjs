#!/usr/bin/env node
/**
 * Validates workflowType consistency: canonical z.enum in schemas-zod.ts (workflowTypeEnum)
...
const schemasPath = join(repoRoot, "packages", "pn-core-mcp", "src", "tools", "schemas-zod.ts");
...
const src = readFileSync(schemasPath, "utf-8").replace(/\r\n/g, "\n");
const wfSrc = readFileSync(workflowsPath, "utf-8").replace(/\r\n/g, "\n");

function extractWorkflowTypeEnum(text) {
  const m = text.match(/export const workflowTypeEnum = z\.enum\(\[([\s\S]*?)\]\)/);
  if (!m) return null;
...
if (!enumList || enumList.length === 0) {
  console.error("validate-workflow-enums: Could not extract workflowTypeEnum from schemas-zod.ts");
  process.exit(1);
}
...
if (publicUnion.length !== enumList.length || publicUnion.some((t) => !enumSet.has(t))) {
  ...
  console.error("  schemas-zod.ts:", enumList);
...
for (const schemaName of [
  ...
  if (idx === -1) {
    console.error(`validate-workflow-enums: missing schema block: ${schemaName}`);
    process.exit(1);
  }
...
console.log(
  "validate-workflow-enums: workflow types consistent across schemas-zod.ts and workflows.ts"
);
process.exit(0);
