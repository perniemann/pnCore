...
| `node scripts/install-consumer-gating.mjs [dir]` | Copy portable trailer `.githooks` (and `--ci` workflow) into a downstream project |
...
| `npm run trajectory:record` | Turn a recorded `workflow_step` run (`PNCORE_RUN_LOG_STATE=1`) into a replay fixture graded in `test:full` (ADR-0019) |
