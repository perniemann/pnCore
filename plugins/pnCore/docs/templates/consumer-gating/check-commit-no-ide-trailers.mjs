 ...
 * Consumer template — keep forbidden-line logic in sync with scripts/check-commit-no-ide-trailers.mjs.
 */
...
function rangeForEnv() {
  ...
  if (event === "pull_request") {
    ...
    if (inActions) {
      console.warn(
        "check-commit-no-ide-trailers: skip in Actions (no PR_BASE/PR_HEAD; run only in the trailer workflow)"
      );
      process.exit(0);
    }
  ...
  if (event === "push") {
    ...
    if (inActions) {
      console.warn(
        "check-commit-no-ide-trailers: skip in Actions (no BEFORE/AFTER; run only in the trailer workflow)"
      );
      process.exit(0);
    }
...
if (bad.length) {
  ...
  console.error("See pn-core://reference/consumer-gating.md; enable: git config core.hooksPath .githooks");
