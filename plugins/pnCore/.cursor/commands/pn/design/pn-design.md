...
Produce design plan: Design Read + dials, page mode (Portfolio/Product/Editorial/Tool/Conversion/Catalog), 3-layer typography (display/reading/utility), tokens, components.
...
5b. **Marketing preflight** — when plan page mode is **Portfolio**, **Product marketing**, or **Editorial** (not Tool/app, Conversion form, or Catalog), run `get_command("pn-preflight")` with **strict** tier. For `pn-design-dna` / embedded studio surfaces, preflight uses **studio** tier. Do not declare done on `SHIP: NO-GO`.
...
- **pn-preflight** — Build substep 5b (marketing page modes); `pn-core://reference/marketing-ship-gate.md`
