...
- **Run logging:** Each `workflow_step` appends to `.pncore/workflow-runs.jsonl` with **`runId`** (plus `workflowPhase`, `parallel`, `taskIds` when present). Set `PNCORE_RUN_LOG=` to disable. With `PNCORE_RUN_LOG_STATE=1` the entry also carries a redacted `state` snapshot so the run can be replayed as a CI fixture (`npm run trajectory:record`, ADR-0019; see `workflow-runs-schema.md`).
