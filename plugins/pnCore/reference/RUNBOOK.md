...
- **Run logging:** Each `workflow_step` appends to `.pncore/workflow-runs.jsonl` with **`runId`** (plus `workflowPhase`, `parallel`, `taskIds` when present). Set `PNCORE_RUN_LOG=` to disable. With `PNCORE_RUN_LOG_STATE=1` the entry also carries a redacted `state` snapshot so the run can be replayed as a CI fixture (`npm run trajectory:record`, ADR-0017; see `workflow-runs-schema.md`). Each entry is also a step span — `stepIndex`, `sinceLastStepMs`, `engineMs` (ADR-0018); `workflow_run_query` with `kinds: ["step"]` / `timeline: true` shows where a run spent its time.
