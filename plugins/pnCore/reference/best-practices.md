...
- **Evals before shipping prompt changes.** Both OpenAI (Promptfoo direction) and Anthropic (capability vs regression suites) treat behavioral evals as non-optional for production prompts. New skills **must** ship `EVAL.yaml` (`pn-core://reference/eval-convention.md`, `npm run scaffold:eval` / `npm run check:evals`) with with/without-skill scenarios and Accuracy × Efficiency tags. A full continuous retrieval/behavioral LLM harness remains a tracked gap — use `pn-rag-evaluation` and workflow skeptic gates before shipping prompt changes.
- **Prefer MCP tools over CLI/API in skill design.** When encoding integrations, reference remote MCP tools (auth/IAM via the host) before shelling out to CLIs or raw HTTP. Offline link hygiene is a merge gate: `npm run check:links`.
